...
namespace GotDotNet.Exslt {
	/// <summary>
	/// Custom XsltContext for resolving EXSLT functions in 
	/// XPath-only environment.
	/// </summary>
	public class ExsltContext : XsltContext {
		...
		private void AddExtensionNamespaces() {		    
            ...
            RemoveNamespace("exsl", ExsltNamespaces.Common); 
            ...
            if((this.SupportedFunctions & ExsltFunctionNamespace.Common) > 0)
                AddNamespace("exsl", ExsltNamespaces.Common);
		...
		public override IXsltContextFunction ResolveFunction(string prefix, string name, 
		    ...
		    switch (LookupNamespace(_nt.Get(prefix))) {
		        case ExsltNamespaces.Common:                    	
		            return GetExtensionFunctionImplementation(exsltCommon, name, argTypes);		                                
